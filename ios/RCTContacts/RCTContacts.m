    ...
    if (iOS9) {
        ...
        if (myError) {
            ...
            if (status == kABAuthorizationStatusDenied || status == kABAuthorizationStatusRestricted) {
                ...
                callback(@[[NSNull null], @"authorized"]);
            } else if (status == kABAuthorizationStatusNotDetermined) {  // 用户还没有决定是否授权你的程序进行访问
                callback(@[[NSNull null], @"undefined"]);
            }
        }
...
RCT_EXPORT_METHOD(getAll:(RCTResponseSenderBlock) callback)
{
    if (iOS9) {
        ...
        [self getAllContactsWithLowerIos9:callback];
    }
}

- (void)getAllContactsWithLowerIos9:(RCTResponseSenderBlock) callback {
    CFErrorRef myError = NULL;
    ABAddressBookRef addressBook = ABAddressBookCreateWithOptions(NULL, &myError);
    CFArrayRef peopleArray = ABAddressBookCopyArrayOfAllPeople(addressBook);
    NSInteger contactsCount = CFArrayGetCount(peopleArray);
    ...
    for (int i = 0; i < contactsCount; i++) {
        ...
        if (phones) {
                ...
                if (num != nil && num.length > 0) {
                    ...
                    if (label != nil && label.length > 0) {
                        if ([label containsString:@"<"]) {
                            NSUInteger back = [label rangeOfString:@"<"].location + 1;
                            label = [label substringFromIndex:back];
                        }
                        if ([label containsString:@">"]) {
                            NSUInteger front = [label rangeOfString:@">"].location;
                            label = [label substringToIndex:front];
                        }
                    }
                    NSDictionary *dictNum = @{@"label": label, @"number": num ?: @""};
                    [phoneNumbers addObject:dictNum];
                }
            }
            ...
            [dict setObject:jobTitle forKey:@"jobTitle"];
